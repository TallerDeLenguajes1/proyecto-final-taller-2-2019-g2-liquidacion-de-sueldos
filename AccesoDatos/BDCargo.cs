...
namespace AccesoDatos
{
    public class BDCargo
    {
        ...
        public bool InsertCargo(Cargo cargo)
        {
            ...
            try
            {
                string qry = "insert into cargos (idPC, idCargo, legajo, funcion, fechaIngreso, fechaBaja, antiguedad) values (@idPC, @idCargo, @legajo, @funcion, @fechaIngreso, @fechaBaja, @antiguedad)";
                using (MySqlCommand cmd = new MySqlCommand(qry, conexion.Conectar()))
                {
                    cmd.Parameters.AddWithValue("@idPC", cargo.IdPC);
                    cmd.Parameters.AddWithValue("@idCargo", cargo.IdCargo);
        ...
        public bool DeleteTipoCargo(TipoCargo tipoCargo)
        {
            ...
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand(qry, conexion.Conectar()))
                {
                    cmd.Parameters.AddWithValue("@idPC", tipoCargo.IdTipoCargo);
                    if (cmd.ExecuteNonQuery() == 1)
                    {
                        estadoQry = true;
                    }
